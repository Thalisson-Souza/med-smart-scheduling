...
class TestMedico(TestCase):
    ...
    def test_deve_permitir_registrar_agendamento_dentro_do_horario_do_atendimento(self):
        medico = Medico("Dr. House", time(8,0), time(12,0))

        self.assertTrue(medico.esta_disponivel_no_horario(time(9,00)))

    def test_deve_permitir_registrar_agendamento_no_limite_inferior_do_horario_de_trabalho(self):
        medico = Medico("Dr. House", time(8,0), time(12,0))

        self.assertTrue(medico.esta_disponivel_no_horario(time(8,0)))

    def test_deve_permitir_registrar_agendamento_no_limite_superior_do_horario_de_trabalho(self):
        medico = Medico("Dr. House", time(8,0), time(12,0))
